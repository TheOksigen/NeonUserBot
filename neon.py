...
class InteractiveTelegramClient(TelegramClient):
    # Original Source
    # https://github.com/LonamiWebs/Telethon/master/telethon_examples/interactive_telegram_client.py
    # #
    ...
    def __init__(self, session_user_id, api_id, api_hash,
        ...
        if not loop.run_until_complete(self.is_user_authorized()):
            if telefon is None:
                user_phone = soru(
                    '[?] Telefon nömrəniz (Nümunə: +994xxxxxxxxx): ')
            else:
                user_phone = telefon
            try:
                ...
                hata(
                    "[!] Səhv nömrə yazdınız nümunədəki kim yazın. Nümunə: +994xxxxxxxxxx")
                exit(1)
            except ValueError:
                hata(
                    "[!] Səhv nömrə yazdınız nümunədəki kimi yazın. Nümunə: +994xxxxxxxxx")
                exit(1)

            while self_user is None:
                code = soru(
                    '[?] Telegramdan gələn beş (5) rəqəmli kodu yazın: ')
                try:
                    self_user =\
                        loop.run_until_complete(self.sign_in(code=code))
                except PhoneCodeInvalidError:
                    hata(
                        "[!] Kodu səhv yazdınız. Zəhmət olmasa yenidən cəhd edin. [Çoxlu cəhd eləmək hesabınızın bloklanmasına səbəb ola bilər]")
                    ...
                    try:
                        ...
                        hata(
                            "[!] 2 addımlı şifrənizi səhv yazdınız. Zəhmət olmasa təkrar cəhd edin [Çoxlu cəhd eləmək hesabınızın bloklanmasına səbəb ola bilər]")


if __name__ == '__main__':
    surum = str(sys.version_info[0]) + "." + str(sys.version_info[1])
    bilgi("Neon String v1\nTelegram: @NeonQurulum\nPython: " + surum +
    ...
    if secim == 2:
        API_ID = soru(
            '[?] API ID\'iniz [Hazır key\'ləri işlətmək üçün boş buraxın]: ')
        if API_ID == "":
        ...
        try:
            cookie = requests.post(
                ...
                data={
                    "phone": numara,
                    "random_hash": rastgele,
                    "password": sifre}).cookies.get_dict()
        except BaseException:
            hata(
                "[!] Böyük ehtimalla kodu səhv yazdız. Zəhmət olmasa Scripti yenidən başlad can ciyer")
            exit(1)
        app = requests.post(
        ...
        if soup.title.string == "Create new application":
            ...
            g_inputs = newsoup.find_all(
                "span", {"class": "form-control input-xlarge uneditable-input"})
            ...
            client = InteractiveTelegramClient(
                StringSession(), app_id, api_hash, numara)
            print(
                "[i] String keyiniz aşağıdakıdır!\n\n\n" +
                client.session.save())

        elif soup.title.string == "App configuration":
            bilgi("[i] Halihazır da Tətbiq yaradılıb. API ID/HASH hazırlanır...")
            g_inputs = soup.find_all(
                "span", {
                    "class": "form-control input-xlarge uneditable-input"})
            app_id = g_inputs[0].string
            api_hash = g_inputs[1].string
            bilgi("[i] Məlumatlar alındı! Zəhmət olmasa bunları bir yerə qeyd edin.\n")
            onemli(f"[i] API ID: {app_id}")
