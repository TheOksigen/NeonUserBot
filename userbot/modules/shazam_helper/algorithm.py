#!/usr/bin/python3
# -*- encoding: Utf-8 -*-
from .signature_format import DecodedMessage, FrequencyBand, FrequencyPeak
from numpy import array as nparray, fft, hanning, log, maximum
...
class RingBuffer(list):
    ...
    def __init__(self, buffer_size: int, default_value: Any = None):
        ...
        if default_value is not None:
            list.__init__(self, [copy(default_value)
                          for position in range(buffer_size)])
        else:
...
class SignatureGenerator:
    ...
    def __init__(self):
        ...
        # Lists of 1025 floats, premultiplied with a Hanning function before
        # being passed through FFT, computed from the ring buffer every new 128
        # samples
    ...
    def get_next_signature(self) -> Optional[DecodedMessage]:
        ...
        while (
            ...
            self.samples_processed >= 128 and (
                ...
                self.next_signature.sample_rate_hz < self.MAX_TIME_SECONDS or sum(
                len(peaks) for peaks in self.next_signature.frequency_band_to_sound_peaks.values()) < self.MAX_PEAKS)):
    ...
    def do_fft(self, batch_of_128_s16le_mono_samples):
        ...
        assert len(fft_results) == 1025 and len(
            excerpt_from_ring_buffer) == 2048 == len(HANNING_MATRIX)

        fft_results = (fft_results.real ** 2 +
                       fft_results.imag ** 2) / (1 << 17)
        ...
        for position in range(1025):
            ...
            for former_fft_num in [-1, -3, -6]:
                ...
                former_fft_output = self.spread_ffts_output[(
                    self.spread_ffts_output.position + former_fft_num) % self.spread_ffts_output.buffer_size]
        ...
        for bin_position in range(10, 1015):
            ...
            if (fft_minus_46[bin_position] >= 1 / 64 and
                ...
                for neighbor_offset in [
                        *range(-10, -3, 3), -3, 1, *range(2, 9, 3)]:

                    max_neighbor_in_fft_minus_49 = max(
                        fft_minus_49[bin_position + neighbor_offset], max_neighbor_in_fft_minus_49)
                ...
                if fft_minus_46[bin_position] > max_neighbor_in_fft_minus_49:
                    ...
                    if fft_minus_46[bin_position] > max_neighbor_in_other_adjacent_ffts:
                        ...
                        peak_magnitude = log(
                            max(1 / 64, fft_minus_46[bin_position])) * 1477.3 + 6144
                        peak_magnitude_before = log(
                            max(1 / 64, fft_minus_46[bin_position - 1])) * 1477.3 + 6144
                        peak_magnitude_after = log(
                            max(1 / 64, fft_minus_46[bin_position + 1])) * 1477.3 + 6144

                        peak_variation_1 = peak_magnitude * 2 - \
                            peak_magnitude_before - peak_magnitude_after
                        peak_variation_2 = (
                        ...
                        self.next_signature.frequency_band_to_sound_peaks[band].append(FrequencyPeak(
                            fft_number, int(peak_magnitude), int(corrected_peak_frequency_bin), 16000))
